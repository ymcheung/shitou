<script lang="ts">
  ...
  const folderThemes = {
    inbox: {
      ...
      idle: "text-zinc-700 hover:bg-indigo-50 hover:text-indigo-700 dark:text-zinc-200 dark:hover:bg-indigo-950/40 dark:hover:text-indigo-100",
    ...
    trash: {
      ...
      idle: "text-zinc-700 hover:bg-zinc-100 hover:text-zinc-950 dark:text-zinc-200 dark:hover:bg-zinc-800 dark:hover:text-zinc-50",
      selected:
        "bg-zinc-800 text-white shadow-sm shadow-zinc-900/10 dark:bg-zinc-200 dark:text-zinc-950",
    ...
    spam: {
      ...
      idle: "text-zinc-700 hover:bg-red-50 hover:text-red-900 dark:text-zinc-200 dark:hover:bg-red-950/40 dark:hover:text-red-100",
      selected:
        "bg-red-600 text-white shadow-sm shadow-red-900/10 dark:bg-red-400 dark:text-red-950",
    ...
    default: {
      ...
      idle: "text-zinc-600 hover:bg-zinc-100 hover:text-zinc-950 dark:text-zinc-300 dark:hover:bg-zinc-700",
      selected:
  ...
  function providerIcon(provider: MailAccount["provider"]) {
    if (provider === "gmail") return GoogleLogoIcon;
    if (provider === "outlook") return WindowsLogoIcon;
...
<aside class="flex min-h-0 min-w-0 flex-col bg-transparent">
  <div class="mail-scrollbar min-h-0 flex-1 overflow-y-auto p-3">
    ...
    {#if appError}
      <div
        class="mb-3 flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-medium text-red-700 dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-200"
      >
          ...
          {#if rootFolder.unreadCount}
            <span
              class={[
                ...
                rootSelected
                  ? sidebarStyles.selectedUnreadBadge
                  : rootTheme.unread,
              ]}
            >
      ...
      {#each accounts as account (account.id)}
        ...
        <section>
          ...
          >
            ...
            >
              <span
                class="grid size-8 shrink-0 place-items-center rounded-lg bg-zinc-100 text-zinc-600 dark:bg-zinc-900 dark:text-zinc-300"
              >
              ...
              {#each folders as folder (folder.id)}
                ...
                <button
                  class={[
                    sidebarStyles.folderButton,
                    folderSelected
                      ? folderThemeStyles.selected
                      : folderThemeStyles.idle,
                  ]}
                  type="button"
                  ...
                  <span class="flex items-center gap-2">
                    <span
                      class={[
                        sidebarStyles.iconChip,
                        folderSelected
                          ? sidebarStyles.selectedIconChip
                          : folderThemeStyles.chip,
                      ]}
                    >
                      <FolderIcon size={15} />
                    </span>
