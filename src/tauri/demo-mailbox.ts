...
let demoMessages: MessageDetail[] = [
  {
    ...
    preview:
      "Password login is disabled and email one-time codes are the only registration path.",
...
function folderMatchesAggregate(folderId: string, aggregateId: string) {
  ...
  return (
    aggregateFolderNames[aggregateId]?.includes(folder.name.toLowerCase()) ??
    false
  );
}
...
export const demoMailbox = {
  ...
  getMessage(messageId: string) {
    return (
      demoMessages.find((message) => message.id === messageId) ??
      demoMessages[0]
    );
  },
  ...
  deleteMessages(messageIds: string[]) {
    ...
    for (const message of demoMessages) {
      ...
      const currentFolder = demoFolders.find(
  ...
  markMessagesSpam(messageIds: string[]) {
    ...
    for (const message of demoMessages) {
      if (!messageIds.includes(message.id)) continue;
      const spam = demoFolders.find(
        (folder) =>
          folder.accountId === message.accountId &&
          folder.name.toLowerCase() === "spam",
      );
      if (spam && message.folderId !== spam.id) {
        message.folderId = spam.id;
        count += 1;
      }
    }
    refreshDemoUnreadCounts();
